...
tiff("Figure6.tiff", 
     ...
     height = 6, 
     units = 'in', 
     res = 500) # Wrapper for creating a high-resolution TIFF graphic
ggplot(data_total_R, 
       aes(x=FacebookFriendsCount, 
           y=AG_6_NET)) + # pointing R to the data.frame / variables of interest
  geom_point() + # create a scatter plot 
  geom_smooth(method=lm) + # add a regression line
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
        axis.title.x = element_text(vjust=-0.5)) + # justifying axis titles
  xlab("Facebook friends") +
  ylab("Minutes spent on mobile / smartphone per week") # axis labels
...
FBFriendsSmartphone <- cbind(data_total_R$FacebookFriendsCount,
                             data_total_R$AG_6_NET)
...
ggplot(FBFriendsSmartphone, 
       aes(x=V1, 
           y=V2)) + # pointing R to the data.frame / variables of interest
geom_point() + # create a scatter plot
...
scale_x_log10(breaks = trans_breaks("log10", 
                                    function(x) 10^x),
                labels = trans_format("log10", 
                                      math_format(10^.x))) + # use a scale that shows the actual values
geom_smooth(method=lm) + # add a regression line
theme(text = element_text(size=18),
      axis.title.y = element_text(vjust=1.3),
      axis.title.x = element_text(vjust=-0.5)) + # title justification
xlab("Facebook friends") +
ylab("Minutes spent on mobile / smartphone per week") # axis labels
...
TWVisitsPhone <- cbind(data_total_R$Q8_Twitter_Reading_Frequency,
                       data_total_R$AG_6_NET)
TWVisitsPhone[TWVisitsPhone==0]<-1 
TWVisitsPhone <- as.data.frame(TWVisitsPhone) # see above
...
ggplot(TWVisitsPhone, aes(x=V1, 
  ...
  scale_x_log10(breaks = trans_breaks("log10", 
                                      function(x) 10^x),
                labels = trans_format("log10", 
                                      math_format(10^.x))) + # show the actual values in the scale
  geom_smooth(method=lm) +  # add a regression line
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
        axis.title.x = element_text(vjust=-0.5)) + # format axis titles
  xlab("Number of Twitter visits per week") +
  ylab("Minutes spent on mobile / smartphone per week") # add axis labels 
dev.off() # wrapper for creating a high-quality TIFF graphic

# FIGURE 9
# Scatter plot for number of Twitter followers vs mobile / smartphone usage, using log scales
TwFollowPhone <- cbind(data_total_R$TwitterFollowersCount,
                       data_total_R$AG_6_NET)
...
tiff("Figure9.tiff", 
     ...
     units = 'in', 
     res = 500) # wrapper for creating a high-quality TIFF graphic
ggplot(TwFollowPhone, 
       aes(x=V1, 
           y=V2)) + # add a regression line
  geom_point() +
  scale_y_log10(breaks = trans_breaks("log10", 
                                      function(x) 10^x),
                labels = trans_format("log10", 
...
TwitterVisitsiPad <- cbind(data_total_R$Q8_Twitter_Reading_Frequency,
                           data_total_R$AG_7_NET)
...
ggplot(TwitterVisitsiPad, 
  ...
  scale_x_log10(breaks = trans_breaks("log10", 
                                      function(x) 10^x),
                labels = trans_format("log10", 
                                      math_format(10^.x))) + # show the actual values on the scales
  geom_smooth(method=lm) + # add a regression line
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
        axis.title.x = element_text(vjust=-0.5)) + # formatting axis titles
  xlab("Number of Twitter visits per week") +
  ylab("Minutes spent on iPads per week") # axis labels
dev.off() # wrapper for creating a high-quality TIFF graphic

# FIGURE 11
# Scatter plot for number of Twitter visits per week vs iPad usage, using log scales
TwitterVisitsNoniPad <- cbind(data_total_R$Q8_Twitter_Reading_Frequency,
                              data_total_R$AG_8_NET)
...
ggplot(TwitterVisitsNoniPad, 
  ...
  theme(text = element_text(size=18),
        ...
        axis.title.x = element_text(vjust=-0.5)) + # formatting axis titles
  xlab("Number of Twitter visits per week") +
  ylab("Minutes spent on non-iPad tablets per week") # add axis labels
dev.off() # wrapper for creating a high-quality TIFF graphic
...
ggplot(data_total_R, 
       aes(x=factor(0), 
           y=data_total_R$AG_8_NET)) + # pointing R to the data.frame / variables of interest
  geom_boxplot() + # create a box plot
  ylab("Non-iPad tablet usage (min per week)") +
  xlab("Overall distribution") + # add axis titles
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
        axis.title.x = element_text(vjust=-0.4, 
                                    size=18), # format and position axis titles
