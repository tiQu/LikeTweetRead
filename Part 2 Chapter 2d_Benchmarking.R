...
FBUpdatesGlobeMail <- data.frame(data_total_R$FacebookUpdatesThirds,
                                 data_total_R$Mosaic_media_ReadOnline_globeandmailcom2)
colnames(FBUpdatesGlobeMail) <- c("FacebookUpdatesThirds",
                                  "globeandmail.com")
...
tiff("Figure14.tiff", 
     ...
     units = 'in', 
     res = 500) # wrapper for creating a high-quality tiff graphic
ggplot(na.omit(FBUpdatesGlobeMail), 
       aes(x = FacebookUpdatesThirds, fill=globeandmail.com)) + 
  geom_bar(position = 'fill', 
           width=.5) + 
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
...
FBLikesHuffPo <- data.frame(data_total_R$FacebookLikesThirds,
                            data_total_R$Mosaic_media_ReadOnline_huffingtonpostca2)
colnames(FBLikesHuffPo) <- c("FacebookLikesThirds",
                             "huffingtonpost.ca")
tiff("Figure15.tiff", 
     width = 10, 
     height = 3, 
     units = 'in', 
     res = 500) # wrapper for creating a high-quality tiff graphic
ggplot(na.omit(FBLikesHuffPo), 
       aes(x = FacebookLikesThirds, 
           fill=huffingtonpost.ca)) + 
  geom_bar(position = 'fill', 
           width=.5) + 
  theme(text = element_text(size=18),
        axis.title.y = element_text(vjust=1.3),
...
FBUpdatesHuffPo <- data.frame(data_total_R$FacebookUpdatesThirds,
                              data_total_R$Mosaic_media_ReadOnline_huffingtonpostca2)
colnames(FBUpdatesHuffPo) <- c("FacebookUpdatesThirds",
                               "huffingtonpost.ca")
tiff("Figure16.tiff", 
     width = 10, 
     height = 3, 
     units = 'in', 
     res = 500) # wrapper for creating a high-quality tiff graphic
ggplot(na.omit(FBUpdatesHuffPo), 
       aes(x = FacebookUpdatesThirds, 
           fill=huffingtonpost.ca)) + 
  geom_bar(position = 'fill', 
           width=.5) + 
  theme(text = element_text(size=18),
...
FBVisitsLocalN <- data.frame(data_total_R$FacebookVisitsThirds,
                             data_total_R$Mosaic_media_ReadOnline_Yourlocaldailynewspaperswebsite2)
colnames(FBVisitsLocalN) <- c("FacebookVisitsThirds",
                              "Local")
tiff("Figure17.tiff", 
     width = 10, 
     height = 4, 
     units = 'in', 
     res = 500) # wrapper for creating a high-quality tiff graphic
ggplot(na.omit(FBVisitsLocalN), 
       aes(x = FacebookVisitsThirds, 
           fill=Local)) + 
  geom_bar(position = 'fill', 
           width=.5) + 
  theme(text = element_text(size=18),
